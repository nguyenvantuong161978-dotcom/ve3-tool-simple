    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            for (var el of spans) {
                ...
                if (text.includes('video') && (text.includes('thành phần') || text.includes('assets'))) {
        ...
        while self._parallel_video_running and not self.stop_flag:
            try:
                ...
                for scene_id, media_id in scene_media_ids.items():
                    ...
                    # Tạo video bằng I2V mode (Chrome ở "Tạo video từ các thành phần")
                    # Chrome tạo đúng reCAPTCHA cho I2V API
                    ok, result_path, error = drission_api.generate_video_i2v_modify_mode(
                        media_id=media_id,
                        prompt=video_prompt,
                        save_path=mp4_path
                    )
